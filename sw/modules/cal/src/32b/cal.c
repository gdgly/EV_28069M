//! \file   ~/sw/modules/cal/src/32b/cal.c
...
#include "sw/modules/cal/src/32b/cal.h"
...
bool CAL_updateState(CAL_Handle handle)
{
  ...
  if(flag_enable)
    {
      uint_least32_t waitTime = CAL_getWaitTime(handle,state);
      uint_least32_t count_state = CAL_getCount_state(handle);
